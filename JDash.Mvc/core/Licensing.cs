...
namespace JDash.Mvc.Core
{
    ...
    internal class DashboardLicense : License
    {
        ...
        public string Product
        {
            get
            {
                ...
                try
                {
                    res = xmlDoc.SelectSingleNode("LicenseData/Product").InnerText;
                    return res;
                }
                catch
                {
            ...
            if (signatureValid)
            {
                if (HttpContext.Current != null)
                {
                    ...
                    else
                    {
                        ...
                        return product.Contains("Asp.Net MVC");
    ...
    internal class DashboardLicenseManager
    {
        ...
        internal int RegisterProduct()
        {
            ...
            if (!string.IsNullOrEmpty(registrationKey))
            {
                try
                {
                    ...
                    Registry.SetValue(@"HKEY_CURRENT_USER\SOFTWARE\Kalitte\JDash", "LicenseInfoMvc", enc);
                    if (res == 0)
        ...
        {
            byte[] existingValue = Registry.GetValue(@"HKEY_CURRENT_USER\SOFTWARE\Kalitte\JDash", "LicenseInfoMvc", null) as byte[];
