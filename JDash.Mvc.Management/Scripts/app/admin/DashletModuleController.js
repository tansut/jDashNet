//angular.module('KltAdmin', ['ui.bootstrap']);
function DashletModuleController($scope, $http, $injector) {
    ...
    self.loadDashlets = function (page) {
        self.when(self.provider.searchDashletModules(self.getFilter(page)),
            ...
            function (err) {
                self.failRequest("Unable to load dashlet modules!");
            }
        );
    }

    self.failRequest = function (message) {
        console.log(err);
        alert(message);
        self.requsestComplated = true;
    }
    ...
    self.updateGroups = function () {
        ...
        function (err) {
            self.failRequest("Unable to load dashlet groups!");
        }
        );
    }
    ...
    self.deleteDashletModule = function (module) {
        ...
        function () {
            alert("Unable to delete dashlet module!");
        }));
    }
    ...
    self.saveDashlet = function (module) {
        if (module.id && module.id.lenght != 0) {
            var module = self.DashletModuleModel(module);
            self.when(self.provider.saveDashletModule(module), self.successSave, self.failSave);
        } else {
            var moduleToOparate = new self.DashletModuleModel(module);
    ...
    $scope.upateModule = function (item, isClone) {
        ...
        if (isClone) {
            $scope.currentModel.title = "Copy of " + $scope.currentModel.title;
        }
    ...
    $scope.bindCheckItems = function () {
        ...
        $scope.dashletRemove = false;
        $scope.dashletClone = false;
    }
